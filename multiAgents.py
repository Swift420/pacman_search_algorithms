...
class ReflexAgent(Agent):
    """
      ...
      The code below is provided as a guide.  You are welcome to chyange
   ...
   #We created a lambda function that takes one argument pos and calls manhattanDistance with newPos and pos as arguments
        distanceToPacman = lambda pos: manhattanDistance(newPos, pos)
        def Ghost(ghost):
            distance =distanceToPacman(ghost.getPosition()) #calculate distance between the ghost and pacman
        ...
        distances_to_food = (distanceToPacman(food) for food in newFood.asList())
        distanceToClosestFood = min(distances_to_food, default=float('inf')) #finding the minimum distance to each food
